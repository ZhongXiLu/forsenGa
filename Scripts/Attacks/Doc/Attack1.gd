    ...
    for direction in directions:
        for direction2 in directions:
            if direction != 0 or direction2 != 0:
                ...
                bullet_instance.position.x = get_parent().global_position.x + (direction * sprite_size_offset)
                bullet_instance.position.y = get_parent().global_position.y + (direction2 * sprite_size_offset)
